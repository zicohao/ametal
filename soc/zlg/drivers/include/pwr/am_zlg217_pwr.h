...
typedef enum am_zlg217_pwr_mode {
    AM_ZLG217_PWR_MODE_RUN = 0,      /**< \brief 运行模式 */
    AM_ZLG217_PWR_MODE_SLEEP,        /**< \brief 睡眠模式 */
    AM_ZLG217_PWR_MODE_STOP,         /**< \brief 停止模式 */
    AM_ZLG217_PWR_MODE_STANBY,       /**< \brief 待机模式 */
...
typedef struct am_zlg217_pwr_mode_init {
    am_zlg217_pwr_mode_t  pwr_mode; /**< \brief 指向 PWR 寄存器块的指针 */
    int                   pin;      /**< \brief 唤醒引脚 */
...
typedef struct am_zlg217_pwr_devinfo {
    ...
    am_zlg217_pwr_mode_init_t *p_pwr_mode;   /**< \brief 指向 PWR 模式数组的指针 */

    am_zlg217_pwr_pvd_info_t  *p_pvd_info;   /**< \brief 指向 PVD 电压监控信息的指针 */
 ...
 *
 * \return 返回 am_zlg217_pwr_handle_t 类型的句柄
 */
 ...
 * \brief 获取系统模式
 *
 * \param[in] mode         PWR 模式定义，值为 am_zlg217_pwr_mode_t 这一枚举类型
 ...
 *       2. 配置 PWR 模式为 AM_ZLG217_PWR_MODE_STANBY 失败后，WKUP 引脚会被配置为下拉
 *          输入模式
 */
 ...
 * \param[in] 无
 *
 * \return am_zlg217_pwr_mode_t 这一枚举类型的值
 */

...
am_local int __rtc_time_get (void *p_drv, am_tm_t *p_tm)
{
    am_zmf159_rtc_dev_t *p_dev    = (am_zmf159_rtc_dev_t *)p_drv;
...
am_local int __rtc_time_set (void *p_drv, am_tm_t *p_tm)
{
    ...
    am_zmf159_rtc_cnt_set(&p_dev->rtc_serv, sec);
...
{
    am_zmf159_rtc_dev_t *p_dev = (am_zmf159_rtc_dev_t *)p_arg;
...
{
    am_zmf159_rtc_dev_t *p_dev    = NULL;
...
am_err_t am_zmf159_rtc_alarm_set (am_rtc_handle_t handle, am_tm_t *p_tm)
{
    ...
    am_zmf159_rtc_alr_set(handle, sec);
                                     ...
                                     void           *p_arg)
{
    ...
    if (AM_ZMF159_RTC_CALLBACK_SECOND == type) {
        ...
        p_dev->p_arg[0] = p_arg;
    } else if (AM_ZMF159_RTC_CALLBACK_ALARM == type) {
        p_dev->pfn_callback[1] = pfn_callback;
        p_dev->p_arg[1] = p_arg;
    } else if (AM_ZMF159_RTC_CALLBACK_OVER == type) {
                                   ...
                                   uint8_t         type)
{
    ...
    if (AM_ZMF159_RTC_CALLBACK_SECOND == type) {
        ...
        p_dev->int_state |= AM_ZMF159_RTC_CALLBACK_SECOND;
    } else if (AM_ZMF159_RTC_CALLBACK_ALARM == type) {
        amhw_zlg217_rtc_crh_allow_int(p_hw_rtc, AMHW_ZLG217_RTC_ALRIE);
        p_dev->int_state |= AM_ZMF159_RTC_CALLBACK_ALARM;
    } else if (AM_ZMF159_RTC_CALLBACK_OVER == type) {
        amhw_zlg217_rtc_crh_allow_int(p_hw_rtc, AMHW_ZLG217_RTC_OWIE);
        p_dev->int_state |= AM_ZMF159_RTC_CALLBACK_OVER;
    } else {
        return -AM_EINVAL;
    }
                                    ...
                                    const am_zmf159_rtc_devinfo_t *p_devinfo)
{
    ...
    p_hw_bkp                = (amhw_zlg_bkp_t *)p_devinfo->bkp_regbase;
...
{
    am_zmf159_rtc_dev_t *p_dev    = (am_zmf159_rtc_dev_t *)handle;
    amhw_zlg_pwr_t   *p_hw_pwr    = NULL;
    ...
    amhw_zlg_bkp_dr_write(ZMF159_BKP, 0, 0);             /* 清除标记 */
    ...
    amhw_zmf159_rcc_bdcr_rtc_disable();                     /* RTC 时钟失能 */

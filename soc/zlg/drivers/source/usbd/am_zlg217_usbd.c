                                                   ...
                                                   am_usbd_endpoint_init_t    *epinit)
{
    ...
    amhw_zlg217_usbd_ep_halt_reset(p_usb, 1 << endpoint);     /* 清除端点暂停 */
    amhw_zlg217_usbd_ep_enable(p_usb, 1 << endpoint);         /* 使能端点 */
                                                    ...
                                                    uint8_t            endpoint)
{
    ...
    amhw_zlg217_usbd_ep_disable(p_usb, 1 << endpoint);  /* 禁能端点 */
                                                      ...
                                                      uint8_t               endpoint_addr)
{
    ...
    amhw_zlg217_usbd_dtog_data0(p_usb, endpoint);

    amhw_zlg217_usbd_ep_halt_reset(p_usb, 1 << endpoint);
...
static am_usb_status_t __usb_device_init (am_usbd_handle_t handle)
{
    ...
    for (i = 0; i < AM_USBD_MAX_EP_CNT - 1; i++) {
        amhw_zlg217_usbd_epx_int_state_clear(p_usb, (zlg217_usb_epx_t)i, AMHW_ZLG217_EPX_INT_STATE_ALL);
    ...
    amhw_zlg217_usbd_active_set(p_usb, ZLG217_USB_ACTIVE);      /* 活跃 */
    amhw_zlg217_usbd_speed_set(p_usb, ZLG217_USB_SPEED_FULL);   /* 全速 */
    amhw_zlg217_usbd_connect_set(p_usb, ZLG217_USB_CONNECT);    /* 连接 */
...
static am_usb_status_t __usb_device_deinit (am_usbd_handle_t handle)
{
    ...
    amhw_zlg217_usbd_int_disable(p_usb, AMHW_ZLG217_USB_INT_EN_ALL); /* 禁能USB中断 */
    amhw_zlg217_usbd_ep_int_disable(p_usb, AMHW_ZLG217_EP_INT_EN_ALL);   /* 禁能端点中断 */
    amhw_zlg217_usbd_ep0_int_disable(p_usb, AMHW_ZLG217_EP0_INT_EN_ALL); /* 禁能端点0的中断 */
    amhw_zlg217_usbd_ep_disable(p_usb, AMHW_ZLG217_USB_EP_ALL);      /* 禁能所有端点 */
    ...
    amhw_zlg217_usbd_epx_int_disable(p_usb, ZLG217_USB_EP2,
                                  AMHW_ZLG217_EPX_INT_EN_ALL);
    amhw_zlg217_usbd_epx_int_disable(p_usb, ZLG217_USB_EP3,
                                  AMHW_ZLG217_EPX_INT_EN_ALL);
    amhw_zlg217_usbd_epx_int_disable(p_usb, ZLG217_USB_EP4,
                                  AMHW_ZLG217_EPX_INT_EN_ALL);
        ...
        while (length) {        /* 如果一次不能完全传输则分多次传输 */
            ...
            if (buffer) {
                for (i = 0; i < send_once_size; i++) {
                    amhw_zlg217_usbd_epx_fifo_write(p_usb, (zlg217_usb_epx2_t)endpoint, *buffer++);
                }
            } else {
                for (i = 0; i < send_once_size; i++) {
                    amhw_zlg217_usbd_epx_fifo_write(p_usb, (zlg217_usb_epx2_t)endpoint, 0);
                }
            }
            amhw_zlg217_usbd_epx_transfer(p_usb, (zlg217_usb_epx2_t)endpoint, send_once_size);
        }
    }
...
static void __ep0_send_empty_packet (am_usbd_handle_t handle)
{
    ...
    case AM_USBD_CTRL_IN:
        while (!amhw_zlg217_usbd_transfer_end(p_usb, ZLG217_USB_EPX0));
        amhw_zlg217_usbd_epx_transfer(p_usb, ZLG217_USB_EPX0, 0);
                                          ...
                                          uint32_t                  length)
{
    ...
    avali_data_cnt = amhw_zlg217_usbd_epx_avail_size_get(p_usb, (zlg217_usb_epx2_t)endpoint);
    ...
    while (avali_data_cnt--) {
        *buffer++ = amhw_zlg217_usbd_epx_fifo_read(p_usb, (zlg217_usb_epx2_t)endpoint);
    }
...
static void __usb_device_setdefault_state(am_zlg227_usbd_dev_t *p_dev)
{
    ...
    amhw_zlg217_usbd_ep_enable(p_usb, AMHW_ZLG217_USB_EP_ALL);  /* 使能端点 */
    ...
    amhw_zlg217_usbd_epx_int_state_clear(p_usb, ZLG217_USB_EP2,
                                         AMHW_ZLG217_EPX_INT_STATE_ALL);
    amhw_zlg217_usbd_epx_int_state_clear(p_usb, ZLG217_USB_EP3,
                                         AMHW_ZLG217_EPX_INT_STATE_ALL);
    ...
    amhw_zlg217_usbd_epx_int_enable(p_usb, ZLG217_USB_EP2,
                                 AMHW_ZLG217_EPX_INT_EN_ALL);
    amhw_zlg217_usbd_epx_int_enable(p_usb, ZLG217_USB_EP3,
                                 AMHW_ZLG217_EPX_INT_EN_ALL);
    amhw_zlg217_usbd_epx_int_enable(p_usb, ZLG217_USB_EP4,
                                 AMHW_ZLG217_EPX_INT_EN_ALL);
}
                                            ...
                                            void                     *param)
{
    ...
    switch ((int)type) {
        ...
        case AM_USBD_CONTROL_STOP:
            amhw_zlg217_usbd_connect_set(p_usb, ZLG217_USB_DISCONNECT); /* 断开连接 */
            error = AM_USB_STATUS_SUCCESS;
            break;
...
static void __usb_setup_handle (am_zlg227_usbd_dev_t *p_dev)
{
    ...
    if(p_usb_dev->state == AM_USBD_CTRL_IDLE) {  // 如果状态为空闲态
        amhw_zlg217_usbd_setupdata_get(p_usb, (uint8_t *)&(p_usb_dev->setup_data)); // 请求数据包
...
static void __usb_device_interrupt_endpoint (am_zlg227_usbd_dev_t *p_dev)
{
    ...
    ep_index = amhw_zlg217_usbd_ep_int_state_get(p_usb);     /* 获得发生中断的端点 */
    amhw_zlg217_usbd_ep_int_state_clear(p_usb, ep_index);    /* 清端点中断 */
    ...
    for (i = 1; i < AM_USBD_MAX_EP_CNT; i++) {
        if (p_dev->device.endpoint_info[i].inuse == AM_TRUE) {
            if ((p_dev->int_ep_union.int_ep_flag >> i) & 1) {
                ...
                ep_int_type = amhw_zlg217_usbd_epx_int_state_get(p_usb, (zlg217_usb_epx_t)(i - 1));
                amhw_zlg217_usbd_epx_int_state_clear(p_usb, (zlg217_usb_epx_t)(i - 1), ep_int_type);
...
static void __usbd_isr_function(void *p_device)
{
    ...
    int_status = amhw_zlg217_usbd_int_state_get(p_usb);     /* 获得中断状态 */
    amhw_zlg217_usbd_int_state_clear(p_usb, int_status);    /* 清除中断状态 */

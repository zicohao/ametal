...
/**
 * \file
 * \brief LPC84x 在应用中编程(IAP)硬件操作接口
 *
...
#ifndef __AMHW_LPC84X_IAP_H
#define __AMHW_LPC84X_IAP_H
...
/**
 * \addtogroup amhw_lpc84x_if_iap
 * \copydoc amhw_lpc84x_iap.h
 * @{
 */

/** IAP ROM API函数 */
typedef void (*amhw_lpc84x_iap_entry_t)(uint32_t[], uint32_t[]);
...
/** \brief 扇区准备工作         */    
#define AMHW_LPC84X_IAP_CMD_PREPARE             50
/** \brief 从RAM复制内容到FLASH */
#define AMHW_LPC84X_IAP_CMD_COPY_RAM_TO_FLASH   51
/** \brief 扇区擦出命令         */
#define AMHW_LPC84X_IAP_CMD_ERASE_SECTOR        52
/** \brief 扇区查空命令         */
#define AMHW_LPC84X_IAP_CMD_BLANK_CHECK         53
/** \brief 读取PartID命令       */
#define AMHW_LPC84X_IAP_CMD_PARTID_READ         54
/** \brief 读取BOOT_VERSION     */
#define AMHW_LPC84X_IAP_CMD_BOOT_VER_READ       55
/** \brief 数据比较指令         */
#define AMHW_LPC84X_IAP_CMD_COMPARE             56
/** \brief 重新进入ISP模式      */
#define AMHW_LPC84X_IAP_CMD_REINVOKE_ISP        57
/** \brief 读UID                */
#define AMHW_LPC84X_IAP_CMD_READ_UID            58
/** \brief 页擦除指令           */
#define AMHW_LPC84X_IAP_CMD_ERASE_PAGE          59
 ...
 * \brief IAP状态
 */
typedef enum amhw_lpc84x_iap_stat {
    
    /** \brief 指令执行成功          */
    AMHW_LPC84X_IAP_STAT_SUCCESS             = 0,
    /** \brief 无效指令              */
    AMHW_LPC84X_IAP_STAT_INVALID_CMD         = 1,
    /** \brief 源地址未按字对齐      */
    AMHW_LPC84X_IAP_STAT_SRC_ADDR_ERROR      = 2,
    /** \brief 目标地址对齐错误      */
    AMHW_LPC84X_IAP_STAT_DST_ADDR_ERROR      = 3,
    /** \brief 源地址不在内存映射中  */
    AMHW_LPC84X_IAP_STAT_SRC_ADDR_NOT_MAPPED = 4,
    /** \brief 目标地址未在内存映射  */
    AMHW_LPC84X_IAP_STAT_DST_ADDR_NOT_MAPPED = 5,
    /** \brief 字节个数值错误        */
    AMHW_LPC84X_IAP_STAT_COUNT_ERROR         = 6,
    /** \brief 扇区值错误            */
    AMHW_LPC84X_IAP_STAT_INVALID_SECTOR      = 7,
    /** \brief 扇区非空              */
    AMHW_LPC84X_IAP_STAT_SECTOR_NOT_BLANK    = 8,
    /** \brief 扇区准备指令没有执行  */
    AMHW_LPC84X_IAP_STAT_SECTOR_NOT_PREPARED = 9,
    /** \brief 源和目标地址数据不同  */
    AMHW_LPC84X_IAP_STAT_COMPARE_ERROR       = 10,
    /** \brief FLASH忙状态           */
    AMHW_LPC84X_IAP_STAT_BUSY                = 11
    
} amhw_lpc84x_iap_stat_t;
 ...
 */
am_static_inline
void amhw_lpc84x_iap_entry (uint32_t cmd_param[], uint32_t status_result[])
{
    ((amhw_lpc84x_iap_entry_t) LPC84X_IAP_ENTRY_LOCATION)(cmd_param,
                                                          status_result);
}
 ...
 *        amhw_lpc84x_iap_prepare()函数进行写入准备操作
 */
 ...
 *       函数前调用amhw_lpc84x_iap_prepare()函数进行准备操作
 */
 ...
 * \param[in] end_sec   : 结束扇区号
 *
 * \return IAP指令执行的状态，执行如果成功(AMHW_LPC84X_IAP_STAT_SUCCESS)表明扇区为空。
 *
 * \note 结束扇区号必须大于或等于起始扇区号
 */

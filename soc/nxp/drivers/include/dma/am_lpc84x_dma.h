 ...
 *
 * DMA 传输的数据通过两种结构描述：
 * - #am_lpc84x_dma_transfer_t  可以进行简单的内存到内存，内存到外设，外设到内存传输；
 * - #am_lpc84x_dma_xfer_desc_t 可以灵活的配置链式传输，如pingpong模式传输。
 * 
 * \internal
...
#ifndef __AM_LPC84X_DMA_H
#define __AM_LPC84X_DMA_H
...
/**
 * \addtogroup am_lpc84x_if_dma
 * \copydoc am_lpc84x_dma.h
 * @{
 */
 ...
 *
 *  用于配置结构体 #am_lpc84x_dma_transfer_t 中flags参数
 * @{
 */
 ...
 *
 * 用于配置 #am_lpc84x_dma_xfer_desc_t 中的xfercfg参数
 *
 * @{
 */
...
#define AM_LPC84X_DMA_STAT_INTA   AM_BIT(0) /**< \brief 中断A发生     */
#define AM_LPC84X_DMA_STAT_INTB   AM_BIT(1) /**< \brief 中断B发生     */
#define AM_LPC84X_DMA_STAT_INTERR AM_BIT(2) /**< \brief 错误中断发生  */
 ...
 *      \arg AM_LPC84X_DMA_STAT_INTA   : 中断A发生
 *      \arg AM_LPC84X_DMA_STAT_INTB   : 中断B发生
 *      \arg AM_LPC84X_DMA_STAT_INTERR : 错误中断发生
 * 
 * \return 无
 *
 * \note #am_lpc84x_dma_transfer() 传输数据不会产生AM_LPC84X_DMA_STAT_INTB中断
 */
 ...
 *
 * 所有传输基于控制器 \n
 * 通过 #am_lpc84x_dma_controller_get()     初始化通道和获取控制器
 * 通过 #am_lpc84x_dma_controller_abort()   终止控制器数据传输
 * 通过 #am_lpc84x_dma_controller_release() 释放控制器
 */
typedef struct am_lpc84x_dma_controller {
    uint8_t                      chan;         /**< \brief 通道               */
    uint8_t                      state;        /**< \brief 控制器状态         */
    uint16_t                     ext_info;     /**< \brief 其他信息，内部使用 */
    am_lpc84x_dma_complete_cb_t  pfn_callback; /**< \brief 回调函数           */

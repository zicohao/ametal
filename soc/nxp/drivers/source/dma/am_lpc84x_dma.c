...
#define __DMA_CHAN_IS_ACTIVE(p_hw_dma,chan) \
           (amhw_lpc84x_dma_chan_isactive((p_hw_dma), (chan))       || \
            amhw_lpc84x_dma_chan_iserror((p_hw_dma), (chan))        || \
            amhw_lpc84x_dma_chan_inta_isactive((p_hw_dma), (chan))  || \
            amhw_lpc84x_dma_chan_intb_isactive((p_hw_dma), (chan)))
...
/** \brief 指向DMA设备的指针 */
am_lpc84x_dma_dev_t *__gp_dma_dev = NULL;
                                                          ...
                                                          uint32_t opts)
{
    ...
    int     slot = -1;
    uint8_t                          trig_out;
    amhw_lpc84x_inmux_dma_trig_src_t trig_src;
    
    am_lpc84x_dma_controller_t      *p_ctr      = NULL;
    const am_lpc84x_dma_devinfo_t   *p_devinfo  = NULL;
    amhw_lpc84x_dma_t               *p_hw_dma   = NULL;
    amhw_lpc84x_inmux_t             *p_hw_inmux = NULL;
    
    if (__gp_dma_dev == NULL || chan > (AMHW_LPC84X_DMA_CHAN_CNT -1 )) {
        return NULL;
    }
    
    p_devinfo  = __gp_dma_dev->p_devinfo; 
    p_hw_dma   = (amhw_lpc84x_dma_t   *)p_devinfo->dma_regbase;
    p_hw_inmux = (amhw_lpc84x_inmux_t *)p_devinfo->inmux_regbase;
    ...
    /* 配置了硬件触发 */
    if ((opts & AMHW_LPC84X_DMA_CHAN_HWTRIG_EN) != 0 ) {
        
        /* 记录配置了硬件触发 */
        AM_BIT_SET(p_ctr->ext_info, __DMA_EXT_INFO_HWTRIG_POS);
        
        /* 配置硬件触发源 */
        trig_src = (amhw_lpc84x_inmux_dma_trig_src_t)AM_BITS_GET(opts, 19, 4);
        amhw_lpc84x_inmux_dma_trig_set(p_hw_inmux, chan, trig_src);
    ...
    /* 配置DMA输出triger复用 */
    if (trig_out != 0) {
        amhw_lpc84x_inmux_dma_mux_set(p_hw_inmux, trig_out, chan);
        AM_BIT_CLR_MASK(opts, AM_SBF(0x03, 23));
    }
    
    if ((opts & AMHW_LPC84X_DMA_CHAN_PERIPH_REQ_EN) != 0) {
    ...
    /*　配置相关信息 */
    amhw_lpc84x_dma_chan_cfg_set(p_hw_dma, chan, opts);
    
    /* 使能通道和通道中断, 准备开始DMA传输 */
    amhw_lpc84x_dma_chan_int_enable(p_hw_dma, chan);
    amhw_lpc84x_dma_chan_enable(p_hw_dma, chan);
...
int am_lpc84x_dma_controller_abort (am_lpc84x_dma_controller_t *p_ctr)
{
    ...
    chan     = p_ctr->chan;
    p_hw_dma = (amhw_lpc84x_dma_t*)__gp_dma_dev->p_devinfo->dma_regbase;
    
    amhw_lpc84x_dma_chan_int_disable(p_hw_dma, chan);
    amhw_lpc84x_dma_chan_disable(p_hw_dma, chan);

    /* 等待通道传输unbusy */
    while (amhw_lpc84x_dma_chan_isbusy(p_hw_dma, chan));

    /* 终止通道传输 */
    amhw_lpc84x_dma_chan_abort(p_hw_dma, chan);
    
    /* 清中断标志 */
    amhw_lpc84x_dma_chan_inta_clr(p_hw_dma, chan);
    amhw_lpc84x_dma_chan_intb_clr(p_hw_dma, chan);
    amhw_lpc84x_dma_chan_error_clr(p_hw_dma, chan);
...
int am_lpc84x_dma_transfer (am_lpc84x_dma_controller_t *p_ctr,
                            ...
                            am_lpc84x_dma_complete_cb_t pfn_callback,
                            void                       *p_arg)
{
    int         state;
    uint32_t    flags = 0;
    
    am_lpc84x_dma_xfer_desc_t xfer_desc;
    ...
    /* 使能A中断和描述符可用标志 */
    flags |= AM_LPC84X_DMA_XFER_SETINTA | AM_LPC84X_DMA_XFER_VALID;
    
    /* 如果没有使能硬件触发，使能软件触发 */
    if (AM_BIT_GET(p_ctr->ext_info,
                   __DMA_EXT_INFO_HWTRIG_POS) == AM_FALSE) {

        flags |= AM_LPC84X_DMA_XFER_SWTRIG;
    }
    
    /* 重复传输，使能自动重载，并连接到自身 */
    if((p_trans->flags & AM_LPC84X_DMA_TRANS_REPEAT) != 0) {

        flags |= AM_LPC84X_DMA_XFER_RELOAD;
        AM_BIT_SET(p_ctr->ext_info, __DMA_EXT_INFO_REPEAT_POS);

    } else {
        
        /* 单次传输，传输完成后清除triger标志 */
        flags |= AM_LPC84X_DMA_XFER_CLRTRIG;
    }
    
    /* 建立通道描述符 */
    state = am_lpc84x_dma_xfer_desc_build(&xfer_desc,        /* 通道描述符       */
    ...
    return am_lpc84x_dma_xfer_desc_startup(p_ctr,
                                           &xfer_desc,
                                           pfn_callback,
                                           p_arg);
}
                                     ...
                                     void                        *p_arg)
{
    ...
    am_lpc84x_dma_xfer_desc_t   *p_desc_tmp   = NULL;
    amhw_lpc84x_dma_t           *p_hw_dma   = NULL;
    ...
    if (AM_BIT_GET(p_ctr->ext_info, __DMA_EXT_INFO_PERIPH_REQ)) {
      ...
      if (amhw_lpc84x_dma_chan_periph_is_enable(p_hw_dma, p_ctr->chan) == AM_FALSE) {
          amhw_lpc84x_dma_chan_periph_enable(p_hw_dma, p_ctr->chan);
      }
    }
    
    p_desc_tmp = (am_lpc84x_dma_xfer_desc_t *)
                  amhw_lpc84x_dma_chan_desc_addr_get(p_hw_dma, chan);
    ...
    if(AM_BIT_GET(p_ctr->ext_info, __DMA_EXT_INFO_REPEAT_POS)) {
        ...
        am_lpc84x_dma_xfer_desc_link(p_desc_tmp, p_desc_tmp);
        AM_BIT_CLR(p_ctr->ext_info, __DMA_EXT_INFO_REPEAT_POS);
    }
    
    amhw_lpc84x_dma_chan_xfercfg_set(p_hw_dma, chan, p_desc_tmp->xfercfg);
...
static void __dma_int_handler (void *p_arg)
{
    ...
    am_lpc84x_dma_controller_t    *p_ctr     = NULL;
    const am_lpc84x_dma_devinfo_t *p_devinfo = __gp_dma_dev->p_devinfo;
    amhw_lpc84x_dma_t             *p_hw_dma  = (amhw_lpc84x_dma_t*)p_devinfo->dma_regbase;
    
    inta   = amhw_lpc84x_dma_inta_flags_get(p_hw_dma);
    intb   = amhw_lpc84x_dma_intb_flags_get(p_hw_dma);
    error  = amhw_lpc84x_dma_error_flags_get(p_hw_dma);

    /* 错误标志 */
    if ((error & 0xFFFFFF) != 0) {
        for (i = 0; i < AMHW_LPC84X_DMA_CHAN_CNT; i++) {
            if (error & AM_BIT(i)) {
                chan = i;
                amhw_lpc84x_dma_chan_error_clr(p_hw_dma, chan);
                amhw_lpc84x_dma_chan_xfercfg_set(p_hw_dma, chan, 0);
                stat = AM_LPC84X_DMA_STAT_INTERR;
                break;
            }
        }
                  ...
                  const am_lpc84x_dma_devinfo_t *p_devinfo)
{
    ...
    amhw_lpc84x_dma_enable(p_hw_dma);
    amhw_lpc84x_dma_sram_addr_set(p_hw_dma, (uint32_t)p_devinfo->p_xfer_base);
...
void am_lpc84x_dma_deinit (void)
{
    ...
    amhw_lpc84x_dma_disable((amhw_lpc84x_dma_t *)p_devinfo->dma_regbase);

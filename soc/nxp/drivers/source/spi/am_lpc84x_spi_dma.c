...
am_local void __spi_cs_on  (am_lpc84x_spi_dma_dev_t *p_this, am_spi_device_t *p_dev);
am_local void __spi_cs_off (am_lpc84x_spi_dma_dev_t *p_this, am_spi_device_t *p_dev);

am_local int  __spi_hard_init (am_lpc84x_spi_dma_dev_t *p_this);
am_local int  __spi_config (am_lpc84x_spi_dma_dev_t *p_this);

am_local int  __spi_dma_trans (am_lpc84x_spi_dma_dev_t *p_dev);
am_local void __spi_dma_isr (void *p_arg, int stat);

am_local int  __spi_mst_sm_event (am_lpc84x_spi_dma_dev_t *p_dev, uint32_t event);
/*******************************************************************************
  SPI驱动函数声明
*******************************************************************************/
 ...
 */
am_local
void __spi_cs_on (am_lpc84x_spi_dma_dev_t *p_this, am_spi_device_t *p_dev)
{
 ...
 */
am_local
void __spi_cs_off (am_lpc84x_spi_dma_dev_t   *p_this,
                   am_spi_device_t  *p_dev)
{
    if (p_this->p_tgl_dev == p_dev) {
        p_this->p_tgl_dev = NULL;
 ...
 */
am_local
int __spi_hard_init (am_lpc84x_spi_dma_dev_t *p_this)
{
...
{
    am_lpc84x_spi_dma_dev_t *p_this   = (am_lpc84x_spi_dma_dev_t *)p_arg;
...
am_local
int __spi_config (am_lpc84x_spi_dma_dev_t *p_this)
{
    amhw_lpc_spi_t  *p_hw_spi  = (amhw_lpc_spi_t *)(p_this->p_devinfo->spi_regbase);
 ...
 */
am_static_inline
void __spi_msg_in (am_lpc84x_spi_dma_dev_t *p_dev, struct am_spi_message *p_msg)
{
 ...
 */
am_static_inline
struct am_spi_message *__spi_msg_out (am_lpc84x_spi_dma_dev_t *p_dev)
{
...
int __spi_msg_start (void              *p_drv,
                     ...
                     am_spi_message_t  *p_msg)
{
    am_lpc84x_spi_dma_dev_t *p_this   = (am_lpc84x_spi_dma_dev_t *)p_drv;
    amhw_lpc_spi_t   *p_hw_spi = (amhw_lpc_spi_t *)(p_this->p_devinfo->spi_regbase);
 ...
 */
am_local
int __spi_dma_trans (am_lpc84x_spi_dma_dev_t *p_dev)
{
    am_lpc84x_spi_dma_dev_t             *p_this      = (am_lpc84x_spi_dma_dev_t *)p_dev;
    const am_lpc84x_spi_dma_devinfo_t   *p_devinfo   = p_this->p_devinfo;
    amhw_lpc_spi_t                      *p_hw_spi    = (amhw_lpc_spi_t *)(p_devinfo->spi_regbase);
    am_spi_transfer_t                   *p_cur_trans = p_dev->p_cur_trans;
    am_lpc84x_dma_controller_t          *p_ctr_tx    = p_this->p_ctr_tx;
    am_lpc84x_dma_controller_t          *p_ctr_rx    = p_this->p_ctr_rx;
    ...
    /* DMA发送通道配置 */
    dma_flags[0] = AM_LPC84X_DMA_XFER_VALID        |    /* 当前通道描述符有效 */
                   AM_LPC84X_DMA_XFER_RELOAD       |    /* 允许重载通道描述符 */
                   AM_LPC84X_DMA_XFER_SWTRIG       |    /* 软件触发 */
                   AM_LPC84X_DMA_XFER_DSTINC_NOINC;     /* 设置目标地址不递增 */

    /* DMA发送结束字通道配置 */
    dma_flags[1] = AM_LPC84X_DMA_XFER_VALID        |    /* 当前通道描述符有效 */
                   AM_LPC84X_DMA_XFER_SWTRIG       |    /* 软件触发 */
                   AM_LPC84X_DMA_XFER_CLRTRIG      |    /* 清除触发标志 */
                   AM_LPC84X_DMA_XFER_DSTINC_NOINC |    /* 设置目标地址不增加 */
                   AM_LPC84X_DMA_XFER_WIDTH_32BIT  |    /* 数据宽度32位 */
                   AM_LPC84X_DMA_XFER_SRCINC_NOINC;     /* 源地址不递增 */


    /* DMA接收通道配置 */
    dma_flags[2] = AM_LPC84X_DMA_XFER_VALID        |      /* 当前配置描述符有效 */
                   AM_LPC84X_DMA_XFER_SWTRIG       |      /* 软件触发 */
                   AM_LPC84X_DMA_XFER_CLRTRIG      |      /* 清除触发标志 */
                   AM_LPC84X_DMA_XFER_SRCINC_NOINC |      /* 源地址不递增 */
                   AM_LPC84X_DMA_XFER_SETINTA;            /* 使能中断A */

    /* 设置DMA数据位宽 */
    if (p_cur_trans->bits_per_word > 8) {
        dma_flags[0] |= AM_LPC84X_DMA_XFER_WIDTH_16BIT;
        dma_flags[2] |= AM_LPC84X_DMA_XFER_WIDTH_16BIT;
        eot_nbyte     = 2;
    } else {
        dma_flags[0] |= AM_LPC84X_DMA_XFER_WIDTH_8BIT;
        dma_flags[2] |= AM_LPC84X_DMA_XFER_WIDTH_8BIT;
        eot_nbyte     = 1;
    }

    /* 发送缓冲区有效 */
    if (p_cur_trans->p_txbuf != NULL) {
        dma_flags[0] |= AM_LPC84X_DMA_XFER_SRCINC_1X;      /* 源地址递增宽度 */
    } else {
        dma_flags[0] |= AM_LPC84X_DMA_XFER_SRCINC_NOINC;   /* 源地址不递增 */
    }

    /* 接收缓冲区有效 */
    if (p_cur_trans->p_rxbuf != NULL) {
        dma_flags[2] |= AM_LPC84X_DMA_XFER_DSTINC_1X;      /* 目标地址递增宽度 */
    } else {
        dma_flags[2] |= AM_LPC84X_DMA_XFER_DSTINC_NOINC ;            /* 目标地址不递增   */
    }
    ...
    if (p_this->dma_trans_num == 1) {
        ...
        /* 建立结束字通道描述符 */
        am_lpc84x_dma_xfer_desc_build(&p_this->g_desc[0],                                  /* 描述符 */
        ...
        if (p_cur_trans->p_rxbuf != NULL) {
            /* 建立RX通道描述符 */
            am_lpc84x_dma_xfer_desc_build(&p_this->g_desc[2],                                  /* 描述符 */
    ...
    /* 发送多次传输时 */
         ...
         if (p_this->dma_trans_num > 1024) {
             ...
             /* 建立结束字通道描述符 */
             am_lpc84x_dma_xfer_desc_build(&p_this->g_desc[1],              /* 描述符 */
                                             (uint32_t)&p_this->p_tx_buf_eot,         /* 源端BUFF */
                                             (uint32_t)&(p_hw_spi->txdatctl), /* 目标BUFF */
                                              4,                              /* TX字节数 */
                                              dma_flags[1]);                  /* 传输配置 */
         ...
         if (p_this->dma_trans_flg == 0) {
             /* DMA连接发送通道 */
             am_lpc84x_dma_xfer_desc_link(&p_this->g_desc[0], &p_this->g_desc[1]);
         }
    }

//    if (amhw_lpc84x_dma_chan_periph_is_enable(AMHW_LPC84X_DMA, p_ctr_tx->chan))
    ...
    /* 启动DMA传输，马上开始传输 */
    am_lpc84x_dma_xfer_desc_startup(p_ctr_rx,
                                    &p_this->g_desc[2],
                                    __spi_dma_isr,
                                    (void *)p_this);

    /* 启动DMA传输，马上开始传输 */
    am_lpc84x_dma_xfer_desc_startup(p_ctr_tx,
                                    &p_this->g_desc[0],
                  __spi_dma_isr,
                                    (void *)p_this);
    return AM_OK;
 ...
 */
am_local
int __spi_mst_sm_event (am_lpc84x_spi_dma_dev_t *p_dev, uint32_t event)
{
 ...
 * \brief SPI 初始化
 */
am_spi_handle_t am_lpc84x_spi_dma_init (am_lpc84x_spi_dma_dev_t           *p_dev,
                                        const am_lpc84x_spi_dma_devinfo_t *p_devinfo)
{
    ...
    /* 配置DMA发送通道 */
    p_dev->p_ctr_tx = am_lpc84x_dma_controller_get(p_devinfo->dma_chan_tx,
                                                   p_devinfo->dma_chan_prio_tx |
                                                   DMA_CHAN_OPT_PERIPH_REQ_EN);
    /* 配置DMA接收通道 */
    p_dev->p_ctr_rx = am_lpc84x_dma_controller_get(p_devinfo->dma_chan_rx,
 ...
 * \brief SPI 去除初始化
 */
void am_lpc84x_spi_dma_deinit (am_spi_handle_t handle)
{
    am_lpc84x_spi_dma_dev_t *p_dev    = (am_lpc84x_spi_dma_dev_t *)handle;
    amhw_lpc_spi_t          *p_hw_spi = (amhw_lpc_spi_t *)(p_dev->p_devinfo->spi_regbase);
    ...
    /* 释放DMA控制器 */
    am_lpc84x_dma_controller_release(p_dev->p_ctr_tx);
    am_lpc84x_dma_controller_release(p_dev->p_ctr_rx);

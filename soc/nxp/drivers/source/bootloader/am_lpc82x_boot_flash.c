 ...
 * \endinternal
 */
#include "am_lpc82x_boot_flash.h"
#include "am_int.h"
...
{
    int key;
    amhw_lpc82x_iap_stat_t ret;
    if(length == 0) {
        return AM_OK;
    }
    am_lpc82x_boot_flash_dev_t *p_dev = (am_lpc82x_boot_flash_dev_t *)p_drv;
    ...
    key = am_int_cpu_lock();
    ret = amhw_lpc82x_iap_prepare(start_sector, end_sector);
    if(ret != AMHW_LPC82X_IAP_STAT_SUCCESS) {
        am_int_cpu_unlock(key);
        return AM_ERROR;
    }
    ret = amhw_lpc82x_iap_erase_sector (start_sector, end_sector);
                                ...
                                uint32_t   size)
{
    ...
    ret = amhw_lpc82x_iap_copy (dst_addr,
                               (uint32_t)p_src,
                                size);
    am_int_cpu_unlock(key);
    if(ret != AMHW_LPC82X_IAP_STAT_SUCCESS) {
...
//    p_dev->flash_info.flash_page_size    = p_devinfo->flash_page_size;

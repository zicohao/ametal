...
int __spi_dma_trans (am_kl26_spi_dma_dev_t *p_dev)
{
    ...
    /* DMA发送通道配置 */
    dma_flags[0] = KL26_DMA_DCR_INTERRUTP_DISABLE        |  /* DMA中断禁能            */
                   KL26_DMA_DCR_PER_REQUEST_ENABLE       |  /* 外设请求使能           */
                   KL26_DMA_DCR_SINGLE_TRANSFERS         |  /* 单次传输               */
                   KL26_DMA_DCR_DESTINATION_INCR_DISABLE |  /* 目的地址自增禁能      */
                   KL26_DMA_DCR_REQUEST_AFFECTED         |  /* 请求有影响             */
                   KL26_DMA_DCR_START_DISABLE    ;           /* DMA软件启动        */

    /* DMA接收通道配置 */
    dma_flags[1] = KL26_DMA_DCR_INTERRUTP_ENABLE         |  /* DMA中断使能            */
                   KL26_DMA_DCR_PER_REQUEST_ENABLE       |  /* 外设请求使能           */
                   KL26_DMA_DCR_SINGLE_TRANSFERS         |  /* 单次传输               */
                   KL26_DMA_DCR_SOURCE_INCR_DISABLE      |  /* 源地址自增禁能        */
                   KL26_DMA_DCR_REQUEST_AFFECTED         |  /* 请求有影响             */
                   KL26_DMA_DCR_START_DISABLE;               /* DMA软件启动        */
    ...
    if (p_cur_trans->p_rxbuf == NULL) {
        ...
        /* 建立接收通道描述符 */
        am_kl26_dma_xfer_desc_build(&(p_this->g_desc[1]),            /* 通道描述符       */
    ...
    if (p_cur_trans->p_txbuf == NULL) {
        ...
        /* 建立发送通道描述符 */
        am_kl26_dma_xfer_desc_build(&(p_this->g_desc[0]),                  /* 通道描述符       */
                                    (uint32_t)(p_txbuf),                   /* 源缓冲区首地址   */
                                    (uint32_t)(&(p_hw_spi->dl)),           /* 目的缓冲区首地址 */
                                    p_cur_trans->nbytes - nbytes_per_word, /* 传输字节数       */
                                    dma_flags[0]);                         /* 传输配置         */
    } else {
        ...
        /* 建立发送通道描述符 */
        am_kl26_dma_xfer_desc_build(&(p_this->g_desc[0]),                  /* 通道描述符       */
                                    (uint32_t)((uint8_t *)p_txbuf + nbytes_per_word),    /* 源缓冲区首地址   */
                                    (uint32_t)(&(p_hw_spi->dl)),           /* 目的缓冲区首地址 */
                                    p_cur_trans->nbytes - nbytes_per_word, /* 传输字节数       */
                                    dma_flags[0]);                         /* 传输配置         */
    }
                                      ...
                                      const am_kl26_spi_dma_devinfo_t *p_devinfo)
{
    ...
    am_kl26_dma_chan_cfg(p_devinfo->dma_chan_rx,
                         ...
                         p_devinfo->dma_src_rx);          /**< \brief 请求源        */

    am_kl26_dma_chan_cfg(p_devinfo->dma_chan_tx,
                         KL26_DMA_TRIGGER_DISABLE |  /**< \brief DMA正常模式   */
                         p_devinfo->dma_src_tx);          /**< \brief 请求源        */

 ...
 * \brief DMA 中断服务程序
 *
 * \param[in] p_arg 用户自定义参数，通过 am_lpc84x_dma_xfer_desc_startup() 函数传递
 * \param[in] state DMA 中断状态标志，由底层驱动传入，该参数的可能取值：
 *                  (#AM_LPC84X_DMA_STAT_INTERR)、(#AM_LPC84X_DMA_STAT_INTA)或
 *                  (#AM_LPC84X_DMA_STAT_INTB)或者它们的或值组合
 *
 * \return 无
 */
...
am_local void __adc_dma_startup (am_lpc84x_dma_controller_t *p_dma_ctr)
{
    ...
    dma_aflags = AM_LPC84X_DMA_XFER_VALID        |  /* 当前通道描述符有效 */
                 ...
                 AM_LPC84X_DMA_XFER_WIDTH_32BIT  |  /* 传输数据宽度：32 位 */
                 AM_LPC84X_DMA_XFER_SRCINC_NOINC |  /* 源端数据地址不递增 */
                 AM_LPC84X_DMA_XFER_DSTINC_1X    |  /* 目标地址按 1x 宽度递增 */
    ...
    am_lpc84x_dma_xfer_desc_build(
       &__g_desc[0],                         /* A 的重载描述符 */
        (uint32_t)(&LPC84X_ADC0->dat0), /* 源端数据地址 */
...
am_local void __adc_config (amhw_lpc84x_adc_t *p_hw_adc, int ch)
{
    ...
    seq_flags = AMHW_LPC84X_ADC_SEQ_CTRL_MODE_BURST   | /* 采用突发转换模式 */
                AMHW_LPC84X_ADC_SEQ_CTRL_TRIG_POL_POS | /* 推荐使用正边沿触发方式 */
                AMHW_LPC84X_ADC_SEQ_CTRL_MODE_EOS     | /* 序列转换完成后触发 DMA */
                AMHW_LPC84X_ADC_SEQ_CTRL_ENA          | /* 序列 A 使能 */
                AMHW_LPC84X_ADC_SEQ_CTRL_ENABLE_CH(ch); /* 使能 ADC 通道 */

...
am_local void __dma_ping_pong_test (amhw_lpc_spi_t *p_hw_spi, uint8_t chan)
{
    ...
    flags1 = AM_LPC82X_DMA_XFER_VALID         | /* 当前通道描述符有效 */
             ...
             AM_LPC82X_DMA_XFER_WIDTH_16BIT   | /* 传输数据宽度：16 bit */
             AM_LPC82X_DMA_XFER_SRCINC_NOINC  | /* 源端数据地址不递增 */
             AM_LPC82X_DMA_XFER_DSTINC_1X     | /* 目标数据地址按 1 个宽度递增 */
    ...
    am_lpc82x_dma_xfer_desc_link(&__g_desc[1], &__g_desc[0]); /* 链接重载描述符 */
    am_lpc82x_dma_xfer_desc_link(&__g_desc[0], &__g_desc[1]); /* 链接重载描述符 */

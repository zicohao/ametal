...
{
    if ((stat & AM_LPC84X_DMA_STAT_INTA) && ((int)p_arg == __TX_DAM_ARG)) {
        while ((__I2C_MASTER->stat & AMHW_LPC_I2C_MASTER_STATE_MASK) !=
                AMHW_LPC_I2C_STAT_MSTTX);

        __g_trans_done_tx = AM_TRUE;
        amhw_lpc_i2c_mst_dma_disable(LPC84X_I2C0);
        amhw_lpc_i2c_mst_stop(LPC84X_I2C0);
    }

    if ((stat & AM_LPC84X_DMA_STAT_INTA) && ((int)p_arg == __RX_DAM_ARG)) {
        while ((__I2C_MASTER->stat & AMHW_LPC_I2C_MASTER_STATE_MASK) !=
...
am_local void __i2c_dma_test (uint8_t addr, uint8_t sub_addr)
{
    ...
    p_ctr = am_lpc84x_dma_controller_get(DMA_CHAN_I2C0_MST_REQ,
                                         DMA_CHAN_OPT_PRIO_3 |
    ...
    am_lpc84x_dma_transfer_build(&transfer,
                                  ...
                                  (uint32_t)&(LPC84X_I2C0->mstdat),
                                  __TEST_LENTH,
                                  AM_LPC84X_DMA_TRANS_WIDTH_8BIT |
                                  AM_LPC84X_DMA_TRANS_SRCINC_1X |
                                  AM_LPC84X_DMA_TRANS_DSTINC_NOINC);

    am_lpc84x_dma_transfer(p_ctr,
                          &transfer,
                           __i2c_dma_isr,
                           (void*)__TX_DAM_ARG);

    __i2c_mst_start(LPC84X_I2C0, addr, sub_addr, __AM_I2C_WR);
    ...
    __i2c_mst_start(LPC84X_I2C0, addr, sub_addr, __AM_I2C_RD);

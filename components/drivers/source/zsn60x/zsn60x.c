...
void zsn60x_uart_data_rx_callback(void *p_arg, char data)
{
    ...
    if((p_dev->rx_count >= p_dev->is_wait) && (p_dev->is_wait != 0)){
        if(p_dev->zsn60x_dev.status == ZSN60X_NORMAL_MODE){
        ...
        }else if((p_dev->zsn60x_dev.status == ZSN60X_AUTO_MODE_NO_INT) ||
            ...
            if(p_dev->is_wait == ZSN60X_FRAME_HEAD_LENGHT){
                ...
                return;
            }else if(p_dev->is_wait >= ZSN60X_FRAME_FRAME_MIN_LENGHT){
                p_dev->is_wait = 0;
                if(p_dev->zsn60x_dev.pfn_card_cb){
                    p_dev->zsn60x_dev.pfn_card_cb(p_dev->zsn60x_dev.p_arg);
                }
                return;
            }
        }
    ...
    if(((p_dev->zsn60x_dev.status == ZSN60X_AUTO_MODE_NO_INT) ||
        ...
        /* 若在ZSN60X_AUTO_MODE_NO_INT 模式下  检测到当前数据为0xb3 则此帧信息可能为自动检测数据*/
        p_dev->rx_count = 0;
        p_dev->buffer[p_dev->rx_count ++] = data;
...
static int __zsn60x_uart_wait_data(zsn60x_uart_dev_t *p_dev, uint32_t nbytes, uint32_t timeout)
{
    if(p_dev->rx_count < nbytes){
        p_dev->is_wait = nbytes;
        return ZSN60X_SEMB_TAKE(p_dev->uart_semb, timeout);
    }else{
        return 0;
    }
}
...
{
    uint8_t            ret   = 0;
    zsn60x_uart_dev_t *p_dev = p_drv;
    ret =  zsn60x_platform_uart_tx(&p_dev->platform_dev, p_cmd_data, cmd_lenght);
    p_dev->rx_count = 0;
    return ret;
}
/**\brief  I2C命令传输函数 */
                                    ...
                                    uint32_t      *p_rx_data_count)
{
    ...
    if(ret != ZSN60X_EXECUTE_SUCCESS){
        ...
        if((tx_data_count < 3) && (org_status != ZSN60X_NORMAL_MODE) && ret == 0){
                                  ...
                                  uint32_t  *p_lenght)
{
    ...
    ret = ZSN60X_SEMB_TAKE(p_dev->int_semb, 800);
    if(ret != ZSN60X_EXECUTE_SUCCESS){
    ...
    if(ret == ZSN60X_EXECUTE_SUCCESS){
        ...
        if(ret == ZSN60X_EXECUTE_SUCCESS){
            *p_lenght = frame_lenght;
            ...
            return ZSN60X_FRAME_INFO_RX_ERR;
        }
    }else{
        return ZSN60X_FRAME_LENGHT_RX_ERR;
    }
}
                                            ...
                                            uint32_t       *p_data_lenght)
{
    ...
    if(count == rx_data_length){
        return ZSN60X_FRAME_NO_SLV_ADDRS_ERR;
    }
    ...
    if(check_sum_rx == check_sum){
        ...
        return ZSN60X_FRAME_CHECK_SUM_ERR;
    }
}
                                             ...
                                             uint32_t  *p_data_lenght)
{
    ...
    zsn60x_uart_dev_t     *p_dev       = (zsn60x_uart_dev_t *)p_drv;\
    ...
    do{
        ret  = __zsn60x_uart_wait_data(p_dev, frame_head_index + 1, 2000);
        frame_head_index ++;
    }while(ret == 0 && (p_rx_data[frame_head_index - 1] != (p_dev->zsn60x_dev.local_addr + 1)));

    frame_head_index -= 1;

    if(ret != 0){
        p_dev->rx_count = 0;
        return ZSN60X_UART_RX_TIMEOUT;
    }

    ret = __zsn60x_uart_wait_data(p_dev, frame_head_index + 8, 500);
    ...
    ret = __zsn60x_uart_wait_data(p_dev, frame_head_index + info_lenght + 10, 500);
...
void zsn60x_i2c_int_pin_hander(void *p_arg)
{
    ...
    if(p_dev->zsn60x_dev.status == ZSN60X_NORMAL_MODE){
        ZSN60X_SEMB_GIVE(p_dev->int_semb);
    }else{
        if(p_dev->zsn60x_dev.pfn_card_cb){
            p_dev->zsn60x_dev.pfn_card_cb(p_dev->zsn60x_dev.p_arg);
        }
...
{
    zsn60x_uart_dev_t *p_dev   = (zsn60x_uart_dev_t *)p_arg;
   if(p_dev->zsn60x_dev.status == ZSN60X_AUTO_MODE_INT){
       if(p_dev->zsn60x_dev.pfn_card_cb){
           p_dev->zsn60x_dev.pfn_card_cb(p_dev->zsn60x_dev.p_arg);
       }
   }
                                  ...
                                  const zsn60x_uart_devinfo_t  *p_devinfo)
{
    ...
    p_dev->rx_count                       = 0;
    p_dev->zsn60x_dev.work_mode           = ZSN60X_WORK_MODE_UART;
    p_dev->zsn60x_dev.status              = ZSN60X_NORMAL_MODE;
    p_dev->zsn60x_dev.local_addr          = p_devinfo->local_addr;

    p_dev->zsn60x_dev.pfn_cmd_tx          = __zsn60x_uart_cmd_tx;
    p_dev->zsn60x_dev.pfn_cmd_rx_analysis = __zsn60x_uart_message_rx_analysis;

    ZSN60X_SEMB_INIT(p_dev->uart_semb);

    if(0 == zsn60x_platform_uart_init(
            &p_dev->platform_dev,
            (const zsn60x_platform_uart_devinfo_t *)&p_devinfo->platform_info,
            &__g_uart_func,
            (void *)p_dev)){

        return (zsn60x_handle_t)&p_dev->zsn60x_dev;
                                 ...
                                 const zsn60x_i2c_devinfo_t *p_devinfo)
{
    ...
    p_dev->zsn60x_dev.work_mode           = ZSN60X_WORK_MODE_IIC;
    p_dev->zsn60x_dev.status              = ZSN60X_NORMAL_MODE;
    p_dev->zsn60x_dev.local_addr          = p_devinfo->local_addr;
    p_dev->zsn60x_dev.pfn_cmd_tx          = __zsn60x_i2c_cmd_tx;
    p_dev->zsn60x_dev.pfn_cmd_rx_analysis = __zsn60x_i2c_message_rx_analysis;

    ZSN60X_SEMB_INIT(p_dev->int_semb);

    if(0 == zsn60x_platform_i2c_init(
            &p_dev->platform_dev,
            (const zsn60x_platform_i2c_devinfo_t *)&p_devinfo->platform_info,
             zsn60x_i2c_int_pin_hander,
            (void *)p_dev)){

        return &p_dev->zsn60x_dev;
                               ...
                               uint32_t            *p_rx_data_count)
{
    return __zsn60x_cmd_execute(handle,
                                handle->local_addr,
                                ZSN60X_DEVICE_CONTROL_CMD,
                                ZSN60X_GET_DEVICE_INFO,
                                NULL, 0,
                            ...
                            uint8_t          isotype)
{
    return __zsn60x_cmd_execute(handle,
                                handle->local_addr,
                                ZSN60X_DEVICE_CONTROL_CMD,
                                ZSN60X_SET_ICC_INTERFACE_PROTOCOL,
                                &isotype, 1,
                                0, NULL, NULL);
}
/** \brief 装载IC卡密钥 */
                            ...
                            uint8_t          antmode_flag)
{
    return __zsn60x_cmd_execute(handle,
                                handle->local_addr,
                                ZSN60X_DEVICE_CONTROL_CMD,
                                ZSN60X_SET_ICC_INTERFACE_OUTPUT,
                                &antmode_flag, 1,
                                0, NULL, NULL);
}
/** \brief 设置天线通道 */
...
{
    uint8_t ret = 0;
    ret = __zsn60x_cmd_execute(handle,
                               handle->local_addr,
                               ZSN60X_DEVICE_CONTROL_CMD,
                               ZSN60X_SET_LOCAL_ADDRESS,
                              &local_addr, 1,
                               0, NULL, NULL);
                              ...
                              uint16_t        *p_atq)
{
    return __zsn60x_cmd_execute(handle,
                                handle->local_addr,
                                ZSN60X_MIFARE_CARD_CMD,
                                ZSN60X_MIFARE_REQUEST,
                               &req_mode, 1,
                           ...
                           zsn60x_auto_detect_ctrl_t   *p_ctrl)
{
    ...
    if(p_ctrl->auth_mode == ZSN60X_AUTO_DETECT_NO_AUTH){
        temp_lenght = 4;
    }else if(p_ctrl->auth_mode == ZSN60X_AUTO_DETECT_E2_AUTH ||
            p_ctrl->auth_mode == ZSN60X_AUTO_DETECT_KEY_AUTH){
        temp[4] = p_ctrl->key_type;
        if(p_ctrl->auth_mode == ZSN60X_AUTO_DETECT_E2_AUTH){
             temp[5] = p_ctrl->key_num;
             temp[6] = p_ctrl->block;
             temp_lenght = 7;
        }else if(p_ctrl->auth_mode == ZSN60X_AUTO_DETECT_KEY_AUTH){
            memcpy(&temp[5], p_ctrl->p_key, p_ctrl->key_len);
            temp[5 + p_ctrl->key_len] = p_ctrl->block;
            temp_lenght = 6 + p_ctrl->key_len;
        }
    }else{
        return ZSN60X_INPUT_PARA_ERROR;
    }

    ret =  __zsn60x_cmd_execute(handle,
                                handle->local_addr,
                                ZSN60X_MIFARE_CARD_CMD,
                                ZSN60X_MIFARE_AUTO_DETECT,
                                temp, temp_lenght,
                                0, NULL, NULL);
    ...
    if(ret == 0){
        ...
        if(handle->work_mode == ZSN60X_WORK_MODE_IIC){
            handle->status = ZSN60X_AUTO_MODE_INT;
        }else{
            if(p_ctrl->ad_mode & ZSN60X_AUTO_DETECT_SEND){
                if(p_ctrl->ad_mode & ZSN60X_AUTO_DETECT_INTERRUPT){
                    handle->status = ZSN60X_AUTO_MODE_INT_MESSAGE;
                }else{
                    handle->status = ZSN60X_AUTO_MODE_NO_INT;
                }
            }else if(p_ctrl->ad_mode & ZSN60X_AUTO_DETECT_INTERRUPT){
                handle->status = ZSN60X_AUTO_MODE_INT;
            }
        }
        handle->is_continue = (p_ctrl->ad_mode & ZSN60X_AUTO_DETECT_CONTINUE) >> 2;
    }
    return ret;
}
/** \brief 获取自动检测数据模式 */
                               ...
                               zsn60x_auto_detect_data_t  *p_data)
{
    ...
    if(handle->is_continue == 0x00){
        handle->status = ZSN60X_NORMAL_MODE;
    }
                             ...
                             uint16_t         *p_atq)
{
    return __zsn60x_cmd_execute(handle,
                                handle->local_addr,
                                ZSN60X_ISO14443_PROTOCOL_CMD,
                                ZSN60X_PICCA_REQUEST,
                               &req_mode, 1,
                              ...
                              uint8_t             *p_all_times)
{
    ...
    ret =  __zsn60x_cmd_execute(handle,
                                handle->local_addr,
                                ZSN60X_DEVICE_CONTROL_CMD,
                                ZSN60X_READ_AUTO_TEST,
                                NULL, 0,
                                3, data, NULL);
    if(ret == 0){
        *p_success_times = data[0];
        *p_fail_times    = data[1];
        *p_all_times     = data[2];
    }
    return ret;

 ...
 */
am_spi_handle_t am_zlg217_spi1_poll_inst_init (void);

/**
 * \brief SPI1 POLL 实例解初始化
 *
 * \param[in] handle 通过 am_zlg217_spi1_poll_inst_init() 函数
 *                   获得的 SPI 标准服务句柄
 *
 * \return 无
 */
void am_zlg217_spi1_poll_inst_deinit (am_spi_handle_t handle);
 ...
 * \return SPI 标准服务句柄，若为 NULL，表明初始化失败
 */
am_spi_handle_t am_zlg217_spi2_poll_inst_init (void);

/**
 * \brief SPI2 POLL 实例解初始化
 *
 * \param[in] handle 通过 am_zlg217_spi2_poll_inst_init() 函数
 *                   获得的 SPI 标准服务句柄
 *
 * \return 无
 */
void am_zlg217_spi2_poll_inst_deinit (am_spi_handle_t handle);
...
/**
 * \brief USBD 键盘 实例解初始化
 *
 * \return USBD 键盘  标准服务句柄，若为 NULL，表明初始化失败
 */
am_usbd_keyboard_handle am_zlg227_usbd_keyboard_inst_init (void);

/**
 * \brief USBD 打印机 实例解初始化
 *
 * \return USBD 打印机  标准服务句柄，若为 NULL，表明初始化失败
 */
am_usbd_printer_handle am_zlg227_usbd_printer_inst_init (void);


/**
 * \brief USBD U盘 实例解初始化
 *
 * \return USBD U盘  标准服务句柄，若为 NULL，表明初始化失败
 */
am_usbd_msc_handle am_zlg227_usb_msc_inst_init (void);

/**
 * \brief USBD CDC  虚拟串口   实例解初始化
 *
 * \return USBD 虚拟串口  标准服务句柄，若为 NULL，表明初始化失败
 */

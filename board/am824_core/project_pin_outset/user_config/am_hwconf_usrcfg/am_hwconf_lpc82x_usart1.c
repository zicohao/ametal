...
am_local void __lpc82x_usart1_plfm_init (void)
{
    ...
    am_gpio_pin_cfg(__U1_TXD_PIN, PIO_FUNC_U1_RXD);
    am_gpio_pin_cfg(__U1_RXD_PIN, PIO_FUNC_U1_TXD);
}
...
am_local void __lpc82x_usart1_plfm_deinit (void)
{
    ...
    am_gpio_pin_cfg(__U1_TXD_PIN, AM_LPC82X_GPIO_FUNC_GPIO);
    am_gpio_pin_cfg(__U1_RXD_PIN, AM_LPC82X_GPIO_FUNC_GPIO);
}

/** \brief USART1 设备信息 */
am_local am_const am_lpc_usart_devinfo_t __g_lpc82x_usart1_devinfo = {
    LPC82X_USART1_BASE,             /* USART1 寄存器块基地址 */
    INUM_USART1,                    /* USART1 中断号 */
    CLK_UART1,                      /* USART1 时钟号 */
    __lpc82x_usart1_plfm_init,      /* 平台初始化函数 */
    __lpc82x_usart1_plfm_deinit,    /* 平台解初始化函数 */
    NULL                            /* 无 RS485 方向控制函数 */
};

 ...
 * \brief WKT 平台初始化
 */
am_local void __lpc84x_wkt_plfm_init (void)
{
 ...
 * \brief WKT 平台解初始化
 */
am_local void __lpc84x_wkt_plfm_deinit (void)
{
    amhw_lpc84x_syscon_periph_reset(AMHW_LPC84X_RESET_WKT);
    amhw_lpc84x_clk_periph_disable(AMHW_LPC84X_CLK_WKT);
}
 ...
 */
am_local
void __lpc84x_wkt_plfm_clk_init (am_const am_lpc_wkt_devinfo_t *p_devinfo)
{
    ...
    case AMHW_LPC_WKT_EXT_CLOCK:
        ...
        /* WAKEUPCLKHYS 引脚迟滞使能 */
        amhw_lpc82x_pmu_wakeupclkhys_enable(LPC84X_PMU);

        /* WAKECLKPAD 引脚使能(使能 PIO0_28 外部时钟功能) */
        amhw_lpc82x_pmu_wakeclkpad_enable(LPC84X_PMU);
...
/** \brief WKT 设备信息 */
am_local am_const am_lpc_wkt_devinfo_t __g_lpc84x_wkt_devinfo = {
    LPC84X_WKT_BASE,                 /* WKT 寄存器块基地址 */
    INUM_WKT,                        /* WKT 中断号 */
    1,                               /* 支持 1 个通道 */
    AMHW_LPC_WKT_LOW_POWER_CLOCK,    /* 选择 IRC 时钟 */
    0,                               /* 使用外部时钟时的频率设置 */
    __lpc84x_wkt_plfm_clk_init,      /* 平台时钟初始化函数 */
    __lpc84x_wkt_plfm_init,          /* 平台初始化函数 */
    __lpc84x_wkt_plfm_deinit         /* 平台解初始化函数 */
};

/** \brief WKT 设备实例 */
am_local am_lpc_wkt_dev_t __g_lpc84x_wkt_dev;
...
{
    return am_lpc_wkt_init(&__g_lpc84x_wkt_dev, &__g_lpc84x_wkt_devinfo);
}

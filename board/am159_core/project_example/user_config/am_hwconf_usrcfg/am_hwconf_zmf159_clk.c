...
/**
 * \file
 * \brief ZMF159 CLK 用户配置文件
 * \sa am_hwconf_zmf159_clk.c
 *
 * \internal
...
/**
 * \addtogroup am_if_src_hwconf_zmf159_clk
 * \copydoc am_hwconf_zmf159_clk.c
 * @{
 */
/** \brief CLK 平台初始化 */
static void __zmf159_clk_plfm_init (void)
{
}

/** \brief CLK设备信息 */
static const am_zmf159_clk_devinfo_t __g_clk_devinfo =
{
    /**
     * \brief HSEOSC外部晶振频率
     *
     ...
     * \brief LSEOSC外部晶振频率
     *
     *  如果rtc_src选择 AMHW_ZMF159_RTCCLK_LSE 则 rtcclk = lse_osc）
     */
    0,

    /** \brief
     *    PLL 时钟源选择
     *    -# AMHW_ZMF159_PLLCLK_HSI_DIV4 : HSI 振荡器 4 分频作为PLL输入时钟
     *    -# AMHW_ZMF159_PLLCLK_HSE      : HSE 作为PLL输入时钟
     */
    AMHW_ZMF159_PLLCLK_HSE,
...
/** \brief 时钟设备实例 */
static am_zmf159_clk_dev_t __g_clk_dev;
...
{
    return am_zmf159_clk_init(&__g_clk_dev, &__g_clk_devinfo);
}
